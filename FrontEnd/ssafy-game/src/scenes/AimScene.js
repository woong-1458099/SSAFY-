...
export default class AimScene extends Phaser.Scene {
  ...
  create() {
    ...
    for (let x = 0; x < W; x += 40) {
      this.add.rectangle(x, H/2, 1, H, 0x112233, 0.5);
    }
    for (let y = 0; y < H; y += 40) {
      this.add.rectangle(W/2, y, W, 1, 0x112233, 0.5);
    }
    ...
    this.add.text(W/2, 10, 'AIM TRAINER', {
    ...
    this.accuracyTxt = this.add.text(W - 20, 30, 'ACC: 100%', {
    ...
    this.crossCircle = this.add.circle(0, 0, 8)
      .setStrokeStyle(2, 0xff4466).setDepth(20);

    this.input.on('pointermove', (pointer) => {
      this.crossH.setPosition(pointer.x, pointer.y);
      this.crossV.setPosition(pointer.x, pointer.y);
      this.crossCircle.setPosition(pointer.x, pointer.y);
    });
    ...
    this.input.on('pointerdown', (pointer) => {
      ...
      const hit = this.targets.some(t =>
        t && t.active &&
        Phaser.Math.Distance.Between(pointer.x, pointer.y, t.x, t.y) < t.radius
      );

      if (!hit) {
        // 미스 클릭 이펙트
        const miss = this.add.text(pointer.x, pointer.y, 'MISS', {
          fontSize: '8px', color: '#ff4466', fontFamily: PF
        }).setOrigin(0.5).setDepth(10);
        this.tweens.add({
          targets: miss, y: pointer.y - 30, alpha: 0,
          duration: 400, onComplete: () => miss.destroy()
        });
        this.combo = 0;
        this.missed++;
        this.updateHUD();
  ...
  spawnTarget() {
    ...
    // 외곽 원
    const outer = this.add.circle(0, 0, radius, 0xff4466, 0.2)
      .setStrokeStyle(3, 0xff4466);
    // 중간 원
    const mid = this.add.circle(0, 0, radius * 0.6, 0xff8888, 0.3)
    ...
    container.add([ch, cv, outer, mid, center]);
    ...
    const shrinkTween = this.tweens.add({
      targets: outer,
      scaleX: 0, scaleY: 0,
      duration: lifeTime, ease: 'Linear'
    });

    const lifeTimer = this.time.delayedCall(lifeTime, () => {
      if (container.active) {
        this.tweens.add({
          targets: container, alpha: 0, scale: 0,
          duration: 150,
          onComplete: () => container.destroy()
        });
        this.targets = this.targets.filter(t => t !== container);
        this.combo = 0;
        this.missed++;
        this.updateHUD();
        this.spawnTarget();
    ...
    container.on('pointerdown', (pointer) => {
      ...
      const dist = Phaser.Math.Distance.Between(
        pointer.x, pointer.y, container.x, container.y
      );
      const distRatio = dist / radius;

      let pts, judgeText, judgeColor;
      if (distRatio < 0.25) {
        pts = 500; judgeText = 'HEADSHOT!'; judgeColor = '#FFD700';
        this.cameras.main.flash(80, 255, 200, 0, false);
      } else if (distRatio < 0.6) {
        pts = 300; judgeText = 'GREAT!'; judgeColor = '#44ff88';
      } else {
        pts = 100; judgeText = 'HIT'; judgeColor = '#4499ff';
      }
      ...
      // 점수 팝업
      const popup = this.add.text(container.x, container.y - 20, `+${pts}`, {
        fontSize: '11px',
        color: judgeColor,
        fontFamily: PF
      }).setOrigin(0.5).setDepth(10);
      ...
      this.tweens.add({
        targets: popup, y: container.y - 70, alpha: 0,
      ...
      // 히트 이펙트
      this.hitEffect(container.x, container.y, judgeColor);
      ...
      // 다음 타겟
      this.time.delayedCall(100, () => this.spawnTarget());
    });

    container.on('pointerover', () => {
      this.crossCircle.setStrokeStyle(2, 0xFFD700);
    });
    container.on('pointerout', () => {
      this.crossCircle.setStrokeStyle(2, 0xff4466);
    });
  ...
  showJudge(text, color) {
    ...
    this.tweens.add({
      targets: this.judgeTxt, alpha: 0, y: 290,
  ...
  updateHUD() {
    ...
    const acc = this.totalShots > 0
      ? Math.floor((this.hits / this.totalShots) * 100) : 100;
  ...
  endGame() {
    ...
    this.add.text(W/2, 120, 'AIM TRAINER', {
      fontSize: '16px', color: '#FFD700', fontFamily: PF
    }).setOrigin(0.5);
    this.add.text(W/2, 152, 'RESULT', {
      fontSize: '10px', color: '#888888', fontFamily: PF
    }).setOrigin(0.5);

    this.add.text(W/2, 210, `${this.score}`, {
      fontSize: '34px', color: '#ffffff', fontFamily: PF
    }).setOrigin(0.5);

    const acc = this.totalShots > 0
      ? Math.floor((this.hits / this.totalShots) * 100) : 0;

    const stats = [
      { label: 'ACCURACY', value: `${acc}%`,         color: '#44ff88' },
      { label: 'MAX COMBO', value: this.maxCombo,    color: '#FFD700' },
      { label: 'HITS',      value: this.hits,         color: '#4499ff' },
      { label: 'MISSED',    value: this.missed,       color: '#ff4466' },
    ];

    stats.forEach((s, i) => {
      this.add.text(W/2 - 150, 275 + i * 38, s.label, {
        fontSize: '8px', color: '#888888', fontFamily: PF
      }).setOrigin(0, 0.5);
      this.add.text(W/2 + 150, 275 + i * 38, String(s.value), {
        fontSize: '10px', color: s.color, fontFamily: PF
      }).setOrigin(1, 0.5);
    });
