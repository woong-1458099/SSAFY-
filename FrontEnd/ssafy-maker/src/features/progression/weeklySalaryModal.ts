...
export function createWeeklySalaryModal(
  ...
  const panel = scene.add
    .rectangle(centerX, centerY, 420, 210, 0x14314f, 0.98)
    .setScrollFactor(0);
  ...
  const buttonBg = scene.add
    .rectangle(centerX, centerY + 60, 154, 46, 0x29527d, 1)
    .setScrollFactor(0);
  ...
  const buttonText = scene.add.text(centerX, centerY + 77, "확인", {
    ...
    color: "#eef7ff",
    resolution: 2
  }).setOrigin(0.5).setPosition(centerX, centerY + 59).setScrollFactor(0);
