...
export function createInventoryPage<EquipmentSlotKey extends string>(scene: Phaser.Scene, options: {
  ...
  createPanelOuterBorder: (centerX: number, centerY: number, width: number, height: number) => Phaser.GameObjects.Rectangle;
  ...
  const inventoryPanel = scene.add.rectangle(
  ...
  const equipCenters = [
    px(equipPanelCenterX - equipSlotGap / 2),
    px(equipPanelCenterX + equipSlotGap / 2)
  ];
