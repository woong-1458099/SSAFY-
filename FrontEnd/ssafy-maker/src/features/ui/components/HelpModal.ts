...
export class HelpModal {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    // Title
    this.titleText = scene.add.text(0, 0, "🎮 게임 도움말", {
    ...
    // Content
    this.contentText = scene.add.text(0, 0, HELP_CONTENT, {
      fontFamily: this.fontFamily,
      fontSize: "13px",
    ...
    this.root.add([this.overlay, this.panel, this.titleText, this.contentText, this.closeButton]);
  ...
  private createCloseButton(): Phaser.GameObjects.Container {
    ...
    bg.fillStyle(HELP_COLORS.closeBtn, 1);
    ...
    container.on("pointerover", () => {
      bg.clear();
      bg.fillStyle(HELP_COLORS.closeBtnHover, 1);
      bg.fillRoundedRect(-50, -16, 100, 32, 6);
    });
  ...
  private updateLayout(): void {
    ...
    this.panel.lineStyle(2, HELP_COLORS.panelBorder, 1);
    this.panel.strokeRoundedRect(panelX, panelY, this.panelWidth, this.panelHeight, 12);

    // Title
    this.titleText.setPosition(panelX + this.panelWidth / 2, panelY + 20);

    // Content
    this.contentText.setPosition(panelX + 24, panelY + 56);

    // Close button
    this.closeButton.setPosition(panelX + this.panelWidth / 2, panelY + this.panelHeight - 30);
  }
}
