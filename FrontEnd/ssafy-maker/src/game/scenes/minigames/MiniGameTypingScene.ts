...
export class MiniGameTypingScene extends Phaser.Scene {
  ...
  create(data: MiniGameData = {}): void {
    ...
    this.add.text(W / 2, 92, "미니게임: 타이핑 훈련", {
      color: "#f4e1b7",
      fontSize: "40px"
    }).setOrigin(0.5);

    this.add.text(W / 2, 138, "단어를 정확히 입력하세요. ESC로 즉시 복귀 가능합니다.", {
      color: "#d3ebff",
      fontSize: "20px"
    }).setOrigin(0.5);

    this.promptText = this.add.text(W / 2, 252, "", {
      color: "#ffffff",
      fontSize: "52px"
    }).setOrigin(0.5);

    this.inputText = this.add.text(W / 2, 336, "", {
      color: "#9df1b9",
      fontSize: "38px"
    }).setOrigin(0.5);
    ...
    this.timerText = this.add.text(W - 190, 54, "남은 시간: 30", {
      color: "#ffffff",
      fontSize: "26px"
    });

    this.finishText = this.add.text(W / 2, 430, "", {
      color: "#ffd88e",
      fontSize: "28px"
    }).setOrigin(0.5);
  ...
  private returnToMain(): void {
    this.scene.stop("MiniGameTypingScene");
