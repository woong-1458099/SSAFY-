...
import {
  ...
  LEGACY_TYPING_INITIAL_WORD_SPEED,
  LEGACY_TYPING_MIN_SPAWN_INTERVAL_MS,
  LEGACY_TYPING_WORDS,
...
export default class TypingScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.typedText = '';
    this.fallingWords = [];
    this.wordSpeed = LEGACY_TYPING_INITIAL_WORD_SPEED;
    this.spawnInterval = LEGACY_TYPING_INITIAL_SPAWN_INTERVAL_MS;
    this.elapsedTime = 0;
  ...
  updateLivesDisplay() {
    ...
    for (let i = 0; i < LEGACY_TYPING_INITIAL_LIVES; i++) {
  ...
  spawnWord() {
    ...
    let wordPool;
    if (difficulty < 0.3) {
      wordPool = LEGACY_TYPING_WORDS.filter(w => w.length <= 5);
    } else if (difficulty < 0.7) {
      wordPool = LEGACY_TYPING_WORDS.filter(w => w.length <= 8);
    } else {
      wordPool = LEGACY_TYPING_WORDS;
    }
  ...
  increaseDifficulty() {
    ...
    // 생성 간격 줄이기
    if (this.spawnInterval > LEGACY_TYPING_MIN_SPAWN_INTERVAL_MS) {
