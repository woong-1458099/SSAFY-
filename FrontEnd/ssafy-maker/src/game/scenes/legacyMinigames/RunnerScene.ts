...
export default class RunnerScene extends Phaser.Scene {
  ...
  spawnObstacle() {
    ...
    const W = 800;
    const obstacleConfig = Phaser.Math.RND.pick(LEGACY_RUNNER_OBSTACLES);
    const obstacle = this.add.container(W + 30, this.groundY + obstacleConfig.yOffset);
    obstacle.add(obstacleConfig.build(this));
    obstacle.height = obstacleConfig.height;
