...
abstract class BaseSmileScene extends Phaser.Scene {
  ...
  protected createCameraSurface(): void {
    ...
    container.style.width = `${LEGACY_SMILE_CAMERA_WIDTH}px`;
    container.style.height = `${LEGACY_SMILE_CAMERA_HEIGHT}px`;
    ...
    canvas.width = LEGACY_SMILE_CAMERA_WIDTH;
    canvas.height = LEGACY_SMILE_CAMERA_HEIGHT;
    canvas.style.display = 'block';
    canvas.style.width = `${LEGACY_SMILE_CAMERA_WIDTH}px`;
    canvas.style.height = `${LEGACY_SMILE_CAMERA_HEIGHT}px`;
  ...
  protected async initFaceTracking(): Promise<void> {
    try {
      ...
      this.cameraInstance = new window.Camera(this.videoElement, {
        ...
        width: LEGACY_SMILE_CAMERA_WIDTH,
        height: LEGACY_SMILE_CAMERA_HEIGHT,
      });
  ...
  protected handleResults(results: FaceMeshResults): void {
    ...
    this.gaugeState.isSmiling = ratio >= LEGACY_SMILE_THRESHOLD;
  ...
  protected drawGauge(ctx: CanvasRenderingContext2D, color: string): void {
    const barWidth = 420;
    const barHeight = 28;
    const x = (LEGACY_SMILE_CAMERA_WIDTH - barWidth) / 2;
    const y = LEGACY_SMILE_CAMERA_HEIGHT - 54;
