...
const LANES = [{ key: 'A', x: 200, color: 0xff4466, darkColor: 0x881133 }, { key: 'S', x: 310, color: 0xffaa00, darkColor: 0x886600 }, { key: 'D', x: 420, color: 0x44ff88, darkColor: 0x228844 }, { key: 'F', x: 530, color: 0x4499ff, darkColor: 0x224488 }];
const HIT_Y = 490, NOTE_SPEED = 300, PERFECT_RANGE = 50, GOOD_RANGE = 90;
...
export default class RhythmScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, 18, SONGS[0].title, { fontSize: '12px', color: '#FFD700', fontFamily: PF }).setOrigin(0.5, 0);
  ...
  handleKey(event) {
    ...
    if (diff < PERFECT_RANGE) { closest.hit = true; closest.obj.destroy(); this.showJudge('PERFECT!', 0xFFD700); this.score += 300 + this.combo * 10; this.perfect += 1; this.combo += 1; this.flashLane(lane); }
    else if (diff < GOOD_RANGE) { closest.hit = true; closest.obj.destroy(); this.showJudge('GOOD', 0x44ff88); this.score += 100 + this.combo * 5; this.good += 1; this.combo += 1; }
  ...
  update() {
    if (this.gameOver || !this.startTime) return;
    const currentTime = this.time.now - this.startTime;
    while (this.songNotes.length > 0 && this.songNotes[0].time <= currentTime + (HIT_Y / NOTE_SPEED) * 1000) this.spawnNote(this.songNotes.shift());
    this.noteObjects.forEach((note) => {
      if (note.hit) return;
      const y = HIT_Y + ((currentTime - note.noteTime) / 1000) * NOTE_SPEED;
      note.obj.setY(y);
  ...
  spawnNote(noteData) {
    ...
    const startY = HIT_Y - (noteData.time / 1000) * NOTE_SPEED + ((this.time.now - this.startTime) / 1000) * NOTE_SPEED;
  ...
  endGame() {
    ...
    this.add.rectangle(W / 2 + 308, H / 2, 4, 420, 0xFFD700);
    this.add.text(W / 2, 110, 'RESULT', { fontSize: '22px', color: '#FFD700', fontFamily: PF }).setOrigin(0.5);
