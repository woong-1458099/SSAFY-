...
export default class TypingScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(W / 2, 10, 'CODE TYPING', { fontSize: '14px', color: '#FFD700', fontFamily: PF }).setOrigin(0.5, 0);
    this.timerTxt = this.add.text(W - 20, 12, 'TIME: 20', { fontSize: '9px', color: '#44ff88', fontFamily: PF }).setOrigin(1, 0);
    ...
    this.add.text(W / 2, 110, `💡  ${this.challenge.desc}`, { fontSize: '11px', color: '#44ff88', fontFamily: PF }).setOrigin(0.5);
    ...
    this.add.rectangle(W / 2 + 360, 210, 3, 74, 0xFFD700);
    this.add.text(60, 195, 'TARGET', { fontSize: '7px', color: '#888888', fontFamily: PF });
    this.add.text(W / 2, 215, this.challenge.code, { fontSize: '18px', color: '#FFD700', fontFamily: 'monospace', align: 'center' }).setOrigin(0.5);
    this.add.rectangle(W / 2 + 2, 322, 724, 74, 0x000000, 0.6);
    this.inputBox = this.add.rectangle(W / 2, 320, 724, 74, 0x0d1117).setStrokeStyle(3, 0x333355);
    this.add.rectangle(W / 2 - 360, 320, 3, 74, 0x333355);
    this.add.rectangle(W / 2 + 360, 320, 3, 74, 0x333355);
    this.add.text(60, 285, 'INPUT', { fontSize: '7px', color: '#888888', fontFamily: PF });
    this.inputDisplay = this.add.text(66, 308, '', { fontSize: '18px', color: '#ffffff', fontFamily: 'monospace' });
    this.cursor = this.add.rectangle(72, 320, 3, 30, 0xffffff);
    this.time.addEvent({ delay: 500, loop: true, callback: () => this.cursor.setVisible(!this.cursor.visible) });
    this.add.text(60, 415, 'PROGRESS', { fontSize: '7px', color: '#888888', fontFamily: PF });
    this.add.rectangle(W / 2, 435, 700, 14, 0x333355);
    this.progressBar = this.add.rectangle(52, 435, 0, 14, 0x44ff88).setOrigin(0, 0.5);
    this.charDisplay = this.add.text(W / 2, 470, '', { fontSize: '14px', color: '#ffffff', fontFamily: 'monospace', align: 'center' }).setOrigin(0.5);
    this.resultTxt = this.add.text(W / 2, 520, '', { fontSize: '13px', color: '#FFD700', fontFamily: PF }).setOrigin(0.5);
    this.statTxt = this.add.text(W / 2, 550, '', { fontSize: '10px', color: '#aaddff', fontFamily: PF }).setOrigin(0.5);
    this.hintTxt = this.add.text(W / 2, 575, 'TYPE THE CODE ABOVE!', { fontSize: '8px', color: '#445566', fontFamily: PF }).setOrigin(0.5);
  ...
  handleKey(event) {
    ...
    if (event.key === 'Backspace') this.typedText = this.typedText.slice(0, -1);
    else if (event.key === 'Enter') { if (this.typedText === this.challenge.code) this.endGame(true); return; }
    else if (event.key.length === 1) this.typedText += event.key;
    this.updateDisplay();
    if (this.typedText === this.challenge.code) this.endGame(true);
  }

  updateDisplay() {
    const target = this.challenge.code, typed = this.typedText;
    this.inputDisplay.setText(typed);
    this.cursor.setX(72 + typed.length * 10.8);
    ...
    if (acc > 0.8) { this.progressBar.setFillStyle(0x44ff88); this.inputBox.setStrokeStyle(3, 0x44ff88); }
    else if (acc > 0.5) { this.progressBar.setFillStyle(0xffaa00); this.inputBox.setStrokeStyle(3, 0xffaa00); }
    else { this.progressBar.setFillStyle(0xff4466); this.inputBox.setStrokeStyle(3, 0xff4466); }
    let charStatus = '';
    for (let i = 0; i < typed.length; i += 1) charStatus += typed[i] === target[i] ? '▪' : '✕';
  ...
  endGame(success) {
    if (this.answered) return;
    this.answered = true; this.timerEvent.remove(); this.input.keyboard.off('keydown', this.handleKey, this); this.cursor.setVisible(false); this.hintTxt.setVisible(false);
    let msg = '⏰ TIME UP...', color = '#ff4466', stat = 'STRESS +5';
    if (success) {
      const elapsed = (Date.now() - this.startTime) / 1000;
      if (elapsed < 7) { msg = '⚡ PERFECT! SPEED CODER!'; color = '#FFD700'; stat = 'INT +7    GP +20'; }
      else if (elapsed < 14) { msg = '✅ SUCCESS!'; color = '#44ff88'; stat = 'INT +5    GP +10'; }
      else { msg = '👍 COMPLETE!'; color = '#aaddff'; stat = 'INT +3    GP +5'; }
      this.cameras.main.flash(200, 0, 255, 100, false);
    } else this.cameras.main.shake(300, 0.008);
    this.resultTxt.setColor(color).setText(msg);
    this.statTxt.setText(stat);
    this.time.delayedCall(600, () => { this.createBtn(270, 575, 'RETRY', 0x001888, 0x4499ff, () => this.scene.restart()); this.createBtn(530, 575, 'MENU', 0x440088, 0xcc55ff, () => this.scene.start('MenuScene')); });
  }

  createBtn(x, y, label, bg, border, cb) {
    this.add.rectangle(x + 3, y + 3, 190, 44, 0x000000, 0.8);
    const btn = this.add.rectangle(x, y, 190, 44, bg).setInteractive().setStrokeStyle(3, border);
