...
<p align="center">
  <img src="./public/assets/game/ui/logo.png" alt="SSAFY생 키우기 로고" width="240" />
</p>

> SSAFY 교육생의 성장 과정을 게임 UX로 풀어낸 웹 기반 시뮬레이션 프로젝트입니다.  
> 이 저장소는 게임 클라이언트를 담당하지만, 실제 서비스는 프론트엔드, 백엔드, 인증, 배포 인프라가 함께 맞물려 동작하는 통합 구조입니다.

## 1. 프로젝트 소개

`SSAFY생 키우기`는 로그인부터 캐릭터 생성, 메인 월드 탐험, NPC 상호작용, 미니게임, 저장/불러오기, 엔딩 분기까지 하나의 플레이 루프로 설계된 웹 게임입니다.

- 교육생의 성장 경험을 게임 플레이로 재해석한 프로젝트
- 브라우저에서 동작하는 `Phaser 3` 기반 2D 게임 클라이언트
...
프론트엔드는 `src/app`, `src/game`, `src/features`, `src/scenes`를 중심으로 구성됩니다.

- 진입점: `src/app/main.ts`, `src/app/game.ts`
- 씬 등록: `src/app/registry/sceneRegistry.ts`
- 핵심 흐름:
...
- 에셋은 `public/assets/game` 아래에서 로드
...
| 이름 | 역할 | 담당 영역 |
| --- | --- | --- |
| 팀원 1 | Frontend | 게임 클라이언트, Phaser 씬, UI/UX |
| 팀원 2 | Frontend | 스토리 연출, 미니게임, 에셋 연동 |
| 팀원 3 | Backend | 인증, 사용자, 세이브/인벤토리 API |
| 팀원 4 | Backend | 챌린지, 대시보드, 데이터 모델링 |
| 팀원 5 | Infra | Docker Compose, Nginx, 배포 파이프라인 |
| 팀원 6 | Infra / DevOps | Jenkins, n8n, 모니터링, 운영 자동화 |

> 6인 팀 기준으로 배치해 둔 템플릿입니다. 실제 이름과 세부 담당만 교체해서 사용하면 됩니다.
...
- [프론트엔드 구현 맵](./docs/FRONTEND_IMPLEMENTATION_MAP.md)
- [환경 설정 가이드](./docs/conventions/ENVIRONMENT_SETUP.md)
- [게임 기획 문서](./docs/game-design/README.md)
